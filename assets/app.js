...
let _dragQId = null, _dropBefore = false;
let _pdfFile = null, _pdfKeyTimer = null;
const state = {
  ...
  newExamType:'builder', pdfForm:{ num_questions:'', num_choices:4 }, pdfFileName:'',
  ...
  _loginU:'', _loginP:'',
  previewMode:false, previewCurrentQ:0, previewAnswers:{}, previewMatchPairs:{}, previewMatchLeft:null,
};
...
async function createExam(){
  ...
  const paper_type=state.newExamType==='pdf'?'pdf':'builder';
  try{ const created=await api('api/exams.php',{method:'POST',body:{title,paper_type}}); toast('สร้างชุดข้อสอบแล้ว'); await loadMyExams(); const ex=state.myExams.find(e=>e.id===created.id)||state.myExams.find(e=>e.title===title); if(ex)openBuilder(ex); }catch(e){toast(e.message,'err');}
}
async function openBuilder(exam){
  _pdfFile=null;
  const patch={currentExam:exam,teacherTab:'builder',questions:[],pdfFileName:''};
  if(exam.paper_type==='pdf') patch.pdfForm={num_questions:'',num_choices:exam.pdf_choices||4};
  setState(patch);
  try{
    const d=await api('api/questions.php?exam_id='+exam.id);
    if(exam.paper_type==='pdf') setState({questions:d,pdfForm:{num_questions:String(d.length||''),num_choices:exam.pdf_choices||4}});
    else setState({questions:d});
  }catch(e){toast(e.message,'err');}
}
// ── PDF-scan exam (teacher) ────────────────────────────────────────────────
function pdfFileSelected(input){ _pdfFile=(input.files&&input.files[0])||null; setState({pdfFileName:_pdfFile?_pdfFile.name:''}); }
async function setupPdfExam(){
  const ex=state.currentExam; if(!ex){toast('กรุณาเลือกชุดข้อสอบก่อน','err');return;}
  const n=parseInt(state.pdfForm.num_questions)||0, c=parseInt(state.pdfForm.num_choices)||0;
  if(n<1||n>300){toast('จำนวนข้อต้องอยู่ระหว่าง 1–300','err');return;}
  if(c<2||c>6){toast('จำนวนตัวเลือกต้องอยู่ระหว่าง 2–6','err');return;}
  if(!ex.pdf_path&&!_pdfFile){toast('กรุณาเลือกไฟล์ข้อสอบ PDF ก่อน','err');return;}
  const fd=new FormData();
  fd.append('exam_paper_id',ex.id); fd.append('num_questions',n); fd.append('num_choices',c);
  if(_pdfFile)fd.append('pdf',_pdfFile);
  const close=loadingModal('กำลังอัปโหลดและสร้างกระดาษคำตอบ...');
  try{
    const res=await fetch('api/exams.php?action=setup_pdf',{method:'POST',credentials:'same-origin',body:fd});
    const data=await res.json();
    if(!res.ok)throw new Error(data.error||('HTTP '+res.status));
    _pdfFile=null;
    const qd=await api('api/questions.php?exam_id='+ex.id);
    await loadMyExams();
    const updated=state.myExams.find(e=>e.id===data.id)||data;
    setState({currentExam:updated,questions:qd,pdfFileName:'',pdfForm:{num_questions:String(qd.length||''),num_choices:updated.pdf_choices||c}});
    toast('ตั้งค่าข้อสอบ PDF เรียบร้อย ('+qd.length+' ข้อ)');
  }catch(e){toast(e.message,'err');}
  finally{close();}
}
function pdfSetKey(qId,label){
  qId=+qId;
  const q=state.questions.find(x=>x.id===qId); if(!q)return;
  const next=q.correct_answer===label?null:label;
  q.correct_answer=next; // mutate data only — no re-render, so the PDF preview stays loaded
  document.querySelectorAll('[data-pdfq="'+qId+'"]').forEach(b=>{
    const on=b.dataset.lb===next;
    b.style.borderColor=on?'var(--st-green-c)':'var(--bdr)';
    b.style.background=on?'var(--st-green-bg)':'var(--bg-card)';
    b.style.color=on?'var(--st-green-c)':'var(--txt-3)';
  });
  const keyed=state.questions.filter(x=>x.correct_answer!=null&&x.correct_answer!=='').length;
  const h=document.getElementById('pdfKeyCount'); if(h)h.textContent='เฉลย ('+keyed+'/'+state.questions.length+')';
  clearTimeout(_pdfKeyTimer); _pdfKeyTimer=setTimeout(()=>savePdfKey(true),700);
}
async function savePdfKey(silent){
  const ex=state.currentExam; if(!ex)return;
  const key={}; state.questions.forEach(q=>{ key[q.id]=q.correct_answer??null; });
  try{ await api('api/exams.php?action=save_pdf_key',{method:'POST',body:{exam_paper_id:ex.id,key}}); if(!silent)toast('บันทึกเฉลยแล้ว'); }
  catch(e){toast(e.message,'err');}
}
...
async function reorderQuestions(newOrder){
  setState({questions:newOrder});
  try{
    await api('api/questions.php?action=reorder',{method:'POST',body:{ids:newOrder.map(q=>q.id)}});
  }catch(e){
    toast(e.message,'err');
    if(state.currentExam){ const d=await api('api/questions.php?exam_id='+state.currentExam.id); setState({questions:d}); }
  }
}
...
function startTimer(seconds){
  clearTimers(); state.timeLeft=seconds; updateClock();
  _timer=setInterval(()=>{
    if(state.timeLeft<=1){ state.timeLeft=0; updateClock(); clearInterval(_timer); _timer=null; submitExam(); return; }
    state.timeLeft--; updateClock();
  },1000);
}
// Tick the countdown via direct DOM so the exam screen (incl. embedded PDF) isn't re-rendered each second.
function updateClock(){
  const el=document.getElementById('examClock'); if(!el)return;
  const m=Math.floor(state.timeLeft/60),s=state.timeLeft%60;
  el.textContent=String(m).padStart(2,'0')+':'+String(s).padStart(2,'0');
  if(state.timeLeft<300){
    el.style.color='var(--st-red-c)';
    const box=document.getElementById('examClockBox'); if(box){ box.style.background='var(--st-red-bg)'; box.style.borderColor='var(--st-red-bdr)'; }
    const ic=document.getElementById('examClockIcon'); if(ic)ic.style.color='var(--st-red-c)';
  }
}
...
// PDF answer-sheet: select a bubble without re-rendering (keeps the embedded PDF from reloading).
function pdfSelAns(qId,label){
  qId=+qId;
  state.answers={...state.answers,[qId]:label};
  document.querySelectorAll('[data-pdfa="'+qId+'"]').forEach(b=>{
    const on=b.dataset.lb===label;
    b.style.borderColor=on?'var(--txt-brand)':'var(--bdr)';
    b.style.background=on?'var(--txt-brand)':'var(--bg-card)';
    b.style.color=on?'#fff':'var(--txt-3)';
  });
  const numEl=document.getElementById('pdfnum_'+qId); if(numEl)numEl.style.color='var(--txt-brand)';
  const answered=Object.keys(state.answers).filter(k=>{const v=state.answers[k];return v!==undefined&&v!==null&&v!=='';}).length;
  const cEl=document.getElementById('pdfAnsCount'); if(cEl)cEl.textContent='ตอบแล้ว '+answered+'/'+(state.questions2||[]).length+' ข้อ';
  const se=state.studentExam;
  if(se)api('api/student.php?action=answer',{method:'POST',body:{student_exam_id:se.id,question_id:qId,answer:label}}).catch(()=>{});
}
// ── Exam preview (teacher sees student view) ───────────────────────────────
function openPreview(){
  const exam=state.currentExam;
  if(!exam){toast('กรุณาเลือกชุดข้อสอบก่อน','err');return;}
  if(!state.questions.length){toast('ยังไม่มีคำถามในชุดข้อสอบนี้','err');return;}
  if(exam.paper_type==='pdf'&&!exam.pdf_path){toast('กรุณาอัปโหลดไฟล์ PDF ก่อน','err');return;}
  setState({previewMode:true,previewCurrentQ:0,previewAnswers:{},previewMatchPairs:{},previewMatchLeft:null});
}
function closePreview(){ setState({previewMode:false}); }
function previewSelOpt(qId,val){ const a={...state.previewAnswers};a[qId]=val;setState({previewAnswers:a}); }
function previewPdfSelAns(qId,label){
  qId=+qId;
  state.previewAnswers={...state.previewAnswers,[qId]:label};
  document.querySelectorAll('[data-prevpdfa="'+qId+'"]').forEach(b=>{
    const on=b.dataset.lb===label;
    b.style.borderColor=on?'var(--txt-brand)':'var(--bdr)';
    b.style.background=on?'var(--txt-brand)':'var(--bg-card)';
    b.style.color=on?'#fff':'var(--txt-3)';
  });
  const numEl=document.getElementById('prevpdfnum_'+qId); if(numEl)numEl.style.color='var(--txt-brand)';
  const answered=Object.keys(state.previewAnswers).filter(k=>{const v=state.previewAnswers[k];return v!==undefined&&v!==null&&v!=='';}).length;
  const cEl=document.getElementById('prevPdfAnsCount'); if(cEl)cEl.textContent='ตอบแล้ว '+answered+'/'+(state.questions||[]).length+' ข้อ';
}
...
const ACTIONS = {
  ...
  createExam,
  newExamType: a=>setState({newExamType:a}),
  setupPdfExam,
  savePdfKey: ()=>savePdfKey(false),
  ...
  noop: ()=>{},
  previewExam: ()=>openPreview(),
  closePreview,
  navPreviewQ:   a=>setState({previewCurrentQ:+a}),
  prevPreviewQ:  ()=>{if(state.previewCurrentQ>0)setState({previewCurrentQ:state.previewCurrentQ-1});},
  nextPreviewQ:  ()=>{if(state.previewCurrentQ<state.questions.length-1)setState({previewCurrentQ:state.previewCurrentQ+1});},
  previewMatchLeft: a=>setState({previewMatchLeft:+a}),
  previewMatchRight: a=>{
    if(state.previewMatchLeft===null)return;
    const q=state.questions[state.previewCurrentQ]; if(!q)return;
    const mp={...state.previewMatchPairs}; mp[state.previewMatchLeft]=+a;
    const na={...state.previewAnswers}; na[q.id]={...mp};
    setState({previewMatchPairs:mp,previewMatchLeft:null,previewAnswers:na});
  },
  startExam,
...
document.getElementById('app').addEventListener('input', e=>{
  ...
  if(f==='pdf_num_q'){state.pdfForm={...state.pdfForm,num_questions:v};return;}
  ...
  if(f==='previewFillAnswer'||f==='previewShortAnswer'){ const q=state.questions[state.previewCurrentQ];if(!q)return; const a={...state.previewAnswers};a[q.id]=v;state.previewAnswers=a; }
});
document.getElementById('app').addEventListener('change', e=>{
  const f=e.target.dataset.field; if(!f)return; const v=e.target.value;
  if(f==='q_tf'){state.qForm={...state.qForm,correct_tf:v==='true'};render();return;}
  if(f==='pdf_num_c'){state.pdfForm={...state.pdfForm,num_choices:+v};render();return;}
...
function wireDragDrop(root){
  const rows=root.querySelectorAll('[data-qid]');
  if(!rows.length)return;
  const clearIndicators=()=>rows.forEach(el=>{el.style.borderTop='';el.style.borderBottom='';el.style.opacity='';});
  rows.forEach(row=>{
    row.addEventListener('dragstart',e=>{
      _dragQId=+row.dataset.qid;
      e.dataTransfer.effectAllowed='move';
      setTimeout(()=>row.style.opacity='0.4',0);
    });
    row.addEventListener('dragend',()=>{ clearIndicators(); _dragQId=null; });
    row.addEventListener('dragover',e=>{
      e.preventDefault();
      if(+row.dataset.qid===_dragQId)return;
      e.dataTransfer.dropEffect='move';
      rows.forEach(el=>{el.style.borderTop='';el.style.borderBottom='';});
      const rect=row.getBoundingClientRect();
      _dropBefore=e.clientY<rect.top+rect.height/2;
      if(_dropBefore) row.style.borderTop='2px solid var(--txt-brand)';
      else            row.style.borderBottom='2px solid var(--txt-brand)';
    });
    ...
    row.addEventListener('drop',e=>{
      ...
      if(!_dragQId||_dragQId===targetId)return;
      const qs=[...state.questions];
      const srcIdx=qs.findIndex(q=>q.id===_dragQId);
      const tgtIdx=qs.findIndex(q=>q.id===targetId);
      if(srcIdx===-1||tgtIdx===-1)return;
      const[moved]=qs.splice(srcIdx,1);
      const newTgt=qs.findIndex(q=>q.id===targetId);
      qs.splice(_dropBefore?newTgt:newTgt+1,0,moved);
      _dragQId=null;
      reorderQuestions(qs);
    });
  });
...
function render(){
  ...
  if(state.previewMode&&state.currentExam&&state.questions.length){
    html+='<div style="position:fixed;inset:0;z-index:9000;display:flex;flex-direction:column;overflow:hidden;">'+renderPreviewOverlay()+'</div>';
  }
  app.innerHTML=html;
...
function renderExamList(){
  ...
  exams.forEach(ex=>{
    ...
    cards+='<div style="background:var(--bg-card);border:1px solid var(--bdr);border-radius:14px;padding:20px;box-shadow:var(--shadow-s);display:flex;flex-direction:column;gap:12px;">'
      ...
      +'<div style="font-size:12px;color:var(--txt-3);margin-top:3px;display:flex;align-items:center;gap:4px;">'+(ex.paper_type==='pdf'?'<span class="msi" style="font-size:14px;color:var(--txt-brand);">picture_as_pdf</span>ข้อสอบ PDF':'<span class="msi" style="font-size:14px;">edit_note</span>สร้างคำถามเอง')+((ex.q_count||ex.question_count)?' · '+(ex.q_count||ex.question_count)+' ข้อ':'')+'</div>'
      +'</div>'
  ...
  return '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:16px;margin-bottom:24px;">'
    +cards
    +'</div>'
    +card('<label style="display:block;font-size:13px;font-weight:600;color:var(--txt-2);margin-bottom:8px;">สร้างชุดข้อสอบใหม่</label>'
      +'<div style="display:flex;gap:8px;margin-bottom:12px;flex-wrap:wrap;">'
      +[['builder','สร้างคำถามเอง','edit_note'],['pdf','อัปโหลดข้อสอบ PDF','picture_as_pdf']].map(p=>{
          const on=state.newExamType===p[0];
          return '<button data-act="newExamType" data-arg="'+p[0]+'" style="display:inline-flex;align-items:center;gap:6px;padding:9px 14px;border-radius:10px;border:1.5px solid '+(on?'var(--txt-brand)':'var(--bdr)')+';background:'+(on?'var(--bg-brand-soft)':'var(--bg-card2)')+';color:'+(on?'var(--txt-brand)':'var(--txt-2)')+';font-size:13px;font-weight:600;cursor:pointer;"><span class="msi" style="font-size:16px;">'+p[2]+'</span>'+p[1]+'</button>';
      }).join('')
      +'</div>'
      ...
      +'</div>'
      +(state.newExamType==='pdf'?'<p style="font-size:12px;color:var(--txt-3);margin-top:10px;display:flex;align-items:center;gap:6px;"><span class="msi" style="font-size:15px;">info</span>หลังสร้าง จะให้อัปโหลดไฟล์ PDF และระบุจำนวนข้อ/ตัวเลือก เพื่อสร้างกระดาษคำตอบ</p>':''));
}
...
function renderBuilder(){
  ...
  if(exam.paper_type==='pdf') return renderPdfBuilder(exam);
  const isPub=exam.status==='published';
  let qListHtml='';
  qs.forEach((q,i)=>{
    const tl=Q_TYPE_LABELS[q.type]||q.type;
    const isEditing=state.editingQId===q.id;
    const numBg=isEditing?'var(--txt-brand)':'var(--st-blue-bg)';
    const numC=isEditing?'#fff':'var(--st-blue-c)';
    qListHtml+='<div draggable="true" data-qid="'+q.id+'" style="display:flex;align-items:center;gap:8px;padding:10px;border-radius:10px;background:'+(isEditing?'var(--bg-brand-soft)':'var(--bg-card2)')+';border:1.5px solid '+(isEditing?'var(--txt-brand)':'transparent')+';margin-bottom:8px;cursor:grab;transition:opacity .15s;">'
      +'<span class="msi" style="font-size:20px;color:var(--txt-4);flex-shrink:0;pointer-events:none;">drag_indicator</span>'
      +'<div style="width:28px;height:28px;border-radius:8px;background:'+numBg+';display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:12px;font-weight:700;color:'+numC+';pointer-events:none;">'+(i+1)+'</div>'
  ...
  return '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;">'
    ...
    +'</div>'
    +'<div style="display:flex;gap:8px;align-items:center;">'
    +(qs.length?btnGhost('previewExam',undefined,'ดูตัวอย่าง','visibility'):'')
...
function renderPdfBuilder(exam){
  const isPub=exam.status==='published';
  const qs=state.questions||[];
  const choices=parseInt(state.pdfForm.num_choices)||exam.pdf_choices||4;
  const hasPdf=!!exam.pdf_path;
  const keyed=qs.filter(q=>q.correct_answer!=null&&q.correct_answer!=='').length;
  ...
  let html='<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;flex-wrap:wrap;gap:12px;">'
    +'<div>'
    +'<div style="font-size:16px;font-weight:700;color:var(--txt-1);display:flex;align-items:center;gap:8px;"><span class="msi" style="font-size:20px;color:var(--txt-brand);">picture_as_pdf</span>'+esc(exam.title)+'</div>'
    +'<div style="font-size:13px;color:var(--txt-3);">ข้อสอบ PDF · '+qs.length+' ข้อ · '+(isPub?'เผยแพร่แล้ว':'ร่าง')+'</div>'
    +'</div>'
    +'<div style="display:flex;gap:8px;align-items:center;">'
    +(hasPdf&&qs.length?btnGhost('previewExam',undefined,'ดูตัวอย่าง','visibility'):'')
    +'<button data-act="publishExam" data-arg="'+exam.id+'" style="padding:8px 16px;border-radius:10px;border:1px solid '+(isPub?'var(--bdr)':'var(--st-green-bdr)')+';background:'+(isPub?'var(--st-amber-bg)':'var(--st-green-bg)')+';color:'+(isPub?'var(--st-amber-c)':'var(--st-green-c)')+';font-size:13px;font-weight:600;cursor:pointer;">'+(isPub?'เปลี่ยนเป็นร่าง':'เผยแพร่ข้อสอบ')+'</button>'
    +'</div>'
    +'</div>';

  const fileBox='<label style="display:flex;align-items:center;gap:10px;padding:14px;border:1.5px dashed var(--bdr);border-radius:12px;cursor:pointer;background:var(--bg-card2);" data-hover="var(--bg-hover)">'
    +'<span class="msi" style="font-size:24px;color:var(--txt-brand);">upload_file</span>'
    +'<span style="font-size:13px;color:var(--txt-2);font-weight:600;flex:1;">'+(state.pdfFileName?esc(state.pdfFileName):(hasPdf?'เปลี่ยนไฟล์ PDF (มีไฟล์อยู่แล้ว)':'เลือกไฟล์ PDF ข้อสอบที่สแกน'))+'</span>'
    +'<input type="file" accept="application/pdf" onchange="pdfFileSelected(this)" style="display:none;">'
    +'</label>';

  const setupCard=card(
    '<h3 style="font-size:14px;font-weight:700;color:var(--txt-1);margin-bottom:14px;">ตั้งค่าข้อสอบ</h3>'
    +fileBox
    +'<div style="display:flex;gap:12px;margin-top:14px;">'
    +'<div style="flex:1;"><label style="display:block;font-size:13px;font-weight:600;color:var(--txt-2);margin-bottom:6px;">จำนวนข้อ</label>'
    +'<input type="number" min="1" max="300" data-fid="pdf_num_q" data-field="pdf_num_q" value="'+esc(state.pdfForm.num_questions)+'" placeholder="เช่น 50" style="'+inputStyle()+'" data-focus="2px solid var(--txt-brand)"></div>'
    +'<div style="flex:1;"><label style="display:block;font-size:13px;font-weight:600;color:var(--txt-2);margin-bottom:6px;">ตัวเลือกต่อข้อ</label>'
    +'<select data-field="pdf_num_c" style="'+inputStyle()+'">'
    +[2,3,4,5,6].map(n=>'<option value="'+n+'"'+(choices===n?' selected':'')+'>'+n+' ตัวเลือก ('+OPTS.slice(0,n).join(' ')+')</option>').join('')
    +'</select></div>'
    +'</div>'
    +'<button data-act="setupPdfExam" style="margin-top:16px;width:100%;display:inline-flex;align-items:center;justify-content:center;gap:8px;padding:11px;background:linear-gradient(135deg,#7B1C1C,#B91C1C);color:#fff;border:none;border-radius:10px;font-size:14px;font-weight:700;cursor:pointer;"><span class="msi" style="font-size:18px;">'+(qs.length?'sync':'auto_awesome')+'</span>'+(qs.length?'อัปเดตข้อสอบ / กระดาษคำตอบ':'สร้างกระดาษคำตอบ')+'</button>'
    +(qs.length?'<p style="font-size:11px;color:var(--txt-3);margin-top:8px;display:flex;align-items:center;gap:6px;line-height:1.5;"><span class="msi" style="font-size:14px;flex-shrink:0;">warning</span>การเปลี่ยนจำนวนข้อจะสร้างกระดาษคำตอบใหม่ (เฉลยเดิมที่ยังใช้ได้จะถูกเก็บไว้)</p>':''));

  const preview=hasPdf
    ? card('<h3 style="font-size:14px;font-weight:700;color:var(--txt-1);margin-bottom:12px;">ตัวอย่างไฟล์ข้อสอบ</h3><object data="'+esc(exam.pdf_path)+'" type="application/pdf" style="width:100%;height:420px;border:1px solid var(--bdr);border-radius:10px;"><p style="font-size:13px;color:var(--txt-3);">ไม่สามารถแสดงตัวอย่าง — <a href="'+esc(exam.pdf_path)+'" target="_blank" style="color:var(--txt-brand);">เปิดไฟล์ PDF</a></p></object>')
    : card('<div style="text-align:center;padding:32px;color:var(--txt-3);"><span class="msi" style="font-size:40px;display:block;margin-bottom:8px;">picture_as_pdf</span><p style="font-size:13px;">ยังไม่มีไฟล์ข้อสอบ อัปโหลดและกดสร้างกระดาษคำตอบ</p></div>');
  ...
  if(qs.length){
    ...
    qs.forEach((q,i)=>{
      ...
      let bubbles='';
      labels.forEach(lb=>{
        const on=q.correct_answer===lb;
        bubbles+='<button data-pdfq="'+q.id+'" data-lb="'+lb+'" onclick="pdfSetKey('+q.id+',\''+lb+'\')" title="เฉลยข้อ '+(i+1)+'" style="width:30px;height:30px;border-radius:50%;border:2px solid '+(on?'var(--st-green-c)':'var(--bdr)')+';background:'+(on?'var(--st-green-bg)':'var(--bg-card)')+';color:'+(on?'var(--st-green-c)':'var(--txt-3)')+';font-size:13px;font-weight:700;cursor:pointer;flex-shrink:0;">'+lb+'</button>';
      });
      rows+='<div style="display:flex;align-items:center;gap:8px;padding:6px 4px;border-bottom:1px solid var(--bdr-s);">'
        +'<span style="width:46px;flex-shrink:0;font-size:13px;font-weight:700;color:var(--txt-2);">ข้อ '+(i+1)+'</span>'
        +'<div style="display:flex;gap:6px;flex-wrap:wrap;">'+bubbles+'</div>'
        +'</div>';
    });
    keyCard=card('<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;">'
      +'<h3 id="pdfKeyCount" style="font-size:14px;font-weight:700;color:var(--txt-1);">เฉลย ('+keyed+'/'+qs.length+')</h3>'
      +btnGhost('savePdfKey',undefined,'บันทึกเฉลย','save')
      +'</div>'
      +'<p style="font-size:12px;color:var(--txt-3);margin-bottom:10px;line-height:1.5;">คลิกตัวเลือกที่เป็นคำตอบที่ถูกต้องของแต่ละข้อ (ระบบบันทึกอัตโนมัติ) — ข้อที่ไม่ใส่เฉลยจะไม่ถูกตรวจให้คะแนนอัตโนมัติ</p>'
      +'<div style="max-height:480px;overflow-y:auto;" data-scroll-id="pdfkey">'+rows+'</div>');
  } else {
    keyCard=card('<div style="text-align:center;padding:32px;color:var(--txt-3);"><span class="msi" style="font-size:40px;display:block;margin-bottom:8px;">checklist</span><p style="font-size:13px;">สร้างกระดาษคำตอบก่อน จึงจะกำหนดเฉลยได้</p></div>');
  }
...
// ── Exam preview overlay (teacher sees student view) ───────────────────────
function renderPreviewOverlay(){
  const exam=state.currentExam;
  const qs=state.questions||[];
  const cq=state.previewCurrentQ||0;
  const q=qs[cq];
  const previewAnswers=state.previewAnswers||{};
  const answered=Object.keys(previewAnswers).filter(k=>{const v=previewAnswers[k];return v!==undefined&&v!==null&&v!=='';}).length;

  const previewBar='<div style="background:linear-gradient(135deg,#1e3a8a,#2563eb);padding:0 20px;height:52px;display:flex;align-items:center;gap:12px;flex-shrink:0;">'
    +'<span class="msi" style="font-size:22px;color:#93c5fd;">visibility</span>'
    +'<span style="font-size:14px;font-weight:700;color:#fff;flex:1;">ตัวอย่าง: '+esc(exam.title)+'</span>'
    +'<span style="background:rgba(255,255,255,0.15);border:1px solid rgba(255,255,255,0.25);border-radius:6px;padding:3px 10px;font-size:11px;font-weight:600;color:rgba(255,255,255,0.9);margin-right:8px;">มุมมองนักเรียน</span>'
    +'<button data-act="closePreview" style="display:inline-flex;align-items:center;gap:6px;padding:7px 14px;background:rgba(255,255,255,0.2);border:1px solid rgba(255,255,255,0.35);border-radius:8px;color:#fff;font-size:13px;font-weight:600;cursor:pointer;" data-hover="rgba(255,255,255,0.3)">'
    +'<span class="msi" style="font-size:16px;">close</span>ปิดตัวอย่าง</button>'
    +'</div>';

  const submitBtn='<button disabled style="width:100%;padding:12px;background:var(--st-gray-bg);color:var(--txt-4);border:1px solid var(--bdr);border-radius:10px;font-size:13px;font-weight:700;cursor:not-allowed;opacity:0.55;">ส่งข้อสอบ (ตัวอย่างเท่านั้น)</button>';

  if(exam.paper_type==='pdf'){
    let rows='';
    qs.forEach((q2,i)=>{
      const labels=Array.isArray(q2.options)?q2.options:[];
      const ans=previewAnswers[q2.id];
      let bubbles='';
      labels.forEach(lb=>{
        const on=ans===lb;
        bubbles+='<button data-prevpdfa="'+q2.id+'" data-lb="'+lb+'" onclick="previewPdfSelAns('+q2.id+',\''+lb+'\')" style="width:34px;height:34px;border-radius:50%;border:2px solid '+(on?'var(--txt-brand)':'var(--bdr)')+';background:'+(on?'var(--txt-brand)':'var(--bg-card)')+';color:'+(on?'#fff':'var(--txt-3)')+';font-size:14px;font-weight:700;cursor:pointer;flex-shrink:0;">'+lb+'</button>';
      });
      const done=ans!==undefined&&ans!==null&&ans!=='';
      rows+='<div style="display:flex;align-items:center;gap:10px;padding:8px 6px;border-bottom:1px solid var(--bdr-s);">'
        +'<span id="prevpdfnum_'+q2.id+'" style="width:38px;flex-shrink:0;font-size:14px;font-weight:700;color:'+(done?'var(--txt-brand)':'var(--txt-3)')+';">'+(i+1)+'.</span>'
        +'<div style="display:flex;gap:8px;flex-wrap:wrap;">'+bubbles+'</div>'
        +'</div>';
    });
    const pdfPane='<div style="flex:1.5;min-width:0;display:flex;flex-direction:column;border-right:1px solid var(--bdr);background:var(--bg-card2);">'
      +(exam.pdf_path
        ?'<object data="'+esc(exam.pdf_path)+'#toolbar=1" type="application/pdf" style="width:100%;flex:1;border:none;"><div style="padding:24px;text-align:center;color:var(--txt-3);">ไม่สามารถแสดง PDF — <a href="'+esc(exam.pdf_path)+'" target="_blank" style="color:var(--txt-brand);font-weight:600;">เปิดไฟล์ข้อสอบ</a></div></object>'
        :'<div style="flex:1;display:flex;align-items:center;justify-content:center;color:var(--txt-3);">ไม่พบไฟล์ข้อสอบ</div>')
      +'</div>';
    const sheetPane='<div style="width:360px;min-width:360px;display:flex;flex-direction:column;background:var(--bg-card);">'
      +'<div style="padding:16px 18px;border-bottom:1px solid var(--bdr);">'
      +'<div style="font-size:15px;font-weight:700;color:var(--txt-1);display:flex;align-items:center;gap:8px;"><span class="msi" style="font-size:20px;color:var(--txt-brand);">ballot</span>กระดาษคำตอบ</div>'
      +'<div id="prevPdfAnsCount" style="font-size:12px;color:var(--txt-3);margin-top:4px;">ตอบแล้ว '+answered+'/'+qs.length+' ข้อ</div>'
      +'</div>'
      +'<div style="flex:1;overflow-y:auto;padding:8px 14px;" data-scroll-id="prevpdfsheet">'+rows+'</div>'
      +'<div style="padding:14px 18px;border-top:1px solid var(--bdr);">'+submitBtn+'</div>'
      +'</div>';
    return previewBar
      +'<div style="flex:1;display:flex;overflow:hidden;background:var(--bg-page);">'+pdfPane+sheetPane+'</div>';
  }

  // Builder exam: question-by-question view
  let qNav='';
  qs.forEach((q2,i)=>{
    const isAns=previewAnswers[q2.id]!==undefined&&previewAnswers[q2.id]!==null&&previewAnswers[q2.id]!=='';
    const isCur=i===cq;
    qNav+='<button data-act="navPreviewQ" data-arg="'+i+'" style="width:36px;height:36px;border-radius:8px;border:1.5px solid '+(isCur?'var(--txt-brand)':(isAns?'var(--st-green-c)':'var(--bdr)'))+';background:'+(isCur?'var(--txt-brand)':(isAns?'var(--st-green-bg)':'var(--bg-card)'))+';color:'+(isCur?'#fff':(isAns?'var(--st-green-c)':'var(--txt-3)'))+';font-size:13px;font-weight:700;cursor:pointer;">'+(i+1)+'</button>';
  });

  let qContent='<div style="font-size:13px;color:var(--txt-3);margin-bottom:6px;">ข้อ '+(cq+1)+' / '+qs.length+(q?' · '+esc(Q_TYPE_LABELS[q.type]||''):'')+'</div>';
  if(!q){
    ...
    if(q.type==='mcq'||q.type==='truefalse'){
      ...
      opts.forEach((o,i)=>{
        const sel=ans===o;
        qContent+='<button data-act="noop" onclick="previewSelOpt('+q.id+',\''+o.replace(/'/g,"\\'")+'\' )" style="display:flex;align-items:center;gap:12px;width:100%;padding:14px;margin-bottom:10px;border-radius:12px;border:2px solid '+(sel?'var(--txt-brand)':'var(--bdr)')+';background:'+(sel?'var(--bg-brand-soft)':'var(--bg-card)')+';text-align:left;cursor:pointer;color:var(--txt-1);font-size:14px;font-weight:'+(sel?'700':'500')+';">'
      ...
      qContent+='<input data-field="previewFillAnswer" value="'+esc(ans||'')+'" placeholder="พิมพ์คำตอบที่นี่..." style="width:100%;padding:14px;border:2px solid var(--bdr);border-radius:12px;font-size:15px;background:var(--bg-input);color:var(--txt-1);outline:none;" data-focus="2px solid var(--txt-brand)">';
    } else if(q.type==='matching'){
      let left=[],right=[];
      try{const op=typeof q.options==='string'?JSON.parse(q.options):q.options;left=op.left||[];right=op.right||[];}catch(e){}
      const mp=state.previewMatchPairs||{};
      const ml=state.previewMatchLeft;
      qContent+='<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">'
        ...
        +left.map((l,i)=>'<button data-act="previewMatchLeft" data-arg="'+i+'" style="display:block;width:100%;padding:12px;margin-bottom:8px;border-radius:10px;border:2px solid '+(ml===i?'var(--txt-brand)':'var(--bdr)')+';background:'+(ml===i?'var(--bg-brand-soft)':'var(--bg-card)')+';text-align:left;cursor:pointer;font-size:14px;color:var(--txt-1);">'+(i+1)+'. '+esc(l)+'</button>').join('')
        +'</div>'
        +'<div><div style="font-size:12px;font-weight:600;color:var(--txt-3);margin-bottom:8px;">คอลัมน์ขวา (คลิกเพื่อจับคู่)</div>'
        +right.map((r,i)=>{
          const pl=Object.entries(mp).find(([li,ri])=>+ri===i);
          return '<button data-act="previewMatchRight" data-arg="'+i+'" style="display:block;width:100%;padding:12px;margin-bottom:8px;border-radius:10px;border:2px solid '+(pl?'var(--st-green-c)':'var(--bdr)')+';background:'+(pl?'var(--st-green-bg)':'var(--bg-card)')+';text-align:left;cursor:pointer;font-size:14px;color:var(--txt-1);">'
            +(pl?'<span style="font-size:11px;color:var(--st-green-c);font-weight:600;">('+( +pl[0]+1)+'→'+(i+1)+') </span>':'')
            +esc(r)+'</button>';
        }).join('')
        +'</div></div>';
    } else if(q.type==='short'||q.type==='drag'){
      qContent+='<textarea data-field="previewShortAnswer" rows="6" placeholder="พิมพ์คำตอบ..." style="width:100%;padding:14px;border:2px solid var(--bdr);border-radius:12px;font-size:15px;background:var(--bg-input);color:var(--txt-1);outline:none;resize:vertical;" data-focus="2px solid var(--txt-brand)">'+esc(ans||'')+'</textarea>';
    }
  }
  ...
  return previewBar
    ...
    +'<div style="border-top:1px solid var(--bdr);padding-top:16px;">'+submitBtn+'</div>'
    +'</div>'
    ...
    +(cq>0?btnGhost('prevPreviewQ',undefined,'ข้อก่อนหน้า','chevron_left'):'<div></div>')
    +(cq<qs.length-1?btnGhost('nextPreviewQ',undefined,'ข้อถัดไป','chevron_right','flex-direction:row-reverse;'):'')
    +'</div>'
    +'</div>'
    +'</div>';
}
...
function renderStudentExam(){
  ...
  const topbar='<div style="background:linear-gradient(135deg,var(--sb-from),var(--sb-to));padding:0 24px;height:60px;display:flex;align-items:center;gap:12px;flex-shrink:0;">'
    ...
    +'<span style="font-size:15px;font-weight:700;color:#fff;flex:1;">'+esc(si.exam_title||si.paper_title||si.title||'ข้อสอบ')+'</span>'
    +themeBtn('border-color:rgba(255,255,255,0.3);background:rgba(255,255,255,0.15);color:#fff;')
    +(!state.examStarted?''
      :'<div id="examClockBox" style="display:flex;align-items:center;gap:8px;padding:8px 16px;border-radius:10px;background:'+(timeLow?'var(--st-red-bg)':'rgba(255,255,255,0.15)')+';border:1px solid '+(timeLow?'var(--st-red-bdr)':'rgba(255,255,255,0.2)')+';">'
      +'<span id="examClockIcon" class="msi" style="font-size:20px;color:'+(timeLow?'var(--st-red-c)':'#fff')+';">timer</span>'
      +'<span id="examClock" style="font-family:\'IBM Plex Mono\',monospace;font-size:18px;font-weight:700;color:'+(timeLow?'var(--st-red-c)':'#fff')+';">'+timeStr+'</span>'
      +'</div>')
    +'</div>';
  ...
  if(!state.examStarted){
    return '<div style="display:flex;flex-direction:column;height:100vh;overflow:hidden;">'
      ...
      +card('<div style="text-align:center;padding:8px;">'
        ...
        +'<h2 style="font-size:20px;font-weight:700;color:var(--txt-1);">'+esc(si.exam_title||si.paper_title||si.title||'ข้อสอบ')+'</h2>'
  ...
  if(si.paper_type==='pdf') return renderStudentPdfExam(si,qs,topbar,answered);
...
// ── Student PDF-scan exam (PDF viewer + virtual answer sheet) ──────────────
function renderStudentPdfExam(si,qs,topbar,answered){
  let rows='';
  qs.forEach((q,i)=>{
    const labels=Array.isArray(q.options)?q.options:[];
    const ans=state.answers[q.id];
    ...
    labels.forEach(lb=>{
      const on=ans===lb;
      bubbles+='<button data-pdfa="'+q.id+'" data-lb="'+lb+'" onclick="pdfSelAns('+q.id+',\''+lb+'\')" style="width:34px;height:34px;border-radius:50%;border:2px solid '+(on?'var(--txt-brand)':'var(--bdr)')+';background:'+(on?'var(--txt-brand)':'var(--bg-card)')+';color:'+(on?'#fff':'var(--txt-3)')+';font-size:14px;font-weight:700;cursor:pointer;flex-shrink:0;">'+lb+'</button>';
    });
    const done=ans!==undefined&&ans!==null&&ans!=='';
    rows+='<div style="display:flex;align-items:center;gap:10px;padding:8px 6px;border-bottom:1px solid var(--bdr-s);">'
      +'<span id="pdfnum_'+q.id+'" style="width:38px;flex-shrink:0;font-size:14px;font-weight:700;color:'+(done?'var(--txt-brand)':'var(--txt-3)')+';">'+(i+1)+'.</span>'
      +'<div style="display:flex;gap:8px;flex-wrap:wrap;">'+bubbles+'</div>'
      +'</div>';
  });
  const pdfPane='<div style="flex:1.5;min-width:0;display:flex;flex-direction:column;border-right:1px solid var(--bdr);background:var(--bg-card2);">'
    +(si.pdf_path
      ?'<object data="'+esc(si.pdf_path)+'#toolbar=1" type="application/pdf" style="width:100%;flex:1;border:none;"><div style="padding:24px;text-align:center;color:var(--txt-3);">ไม่สามารถแสดง PDF ในเบราว์เซอร์นี้ — <a href="'+esc(si.pdf_path)+'" target="_blank" style="color:var(--txt-brand);font-weight:600;">เปิดไฟล์ข้อสอบ</a></div></object>'
      :'<div style="flex:1;display:flex;align-items:center;justify-content:center;color:var(--txt-3);">ไม่พบไฟล์ข้อสอบ</div>')
    +'</div>';
  const sheetPane='<div style="width:360px;min-width:360px;display:flex;flex-direction:column;background:var(--bg-card);">'
    +'<div style="padding:16px 18px;border-bottom:1px solid var(--bdr);">'
    +'<div style="font-size:15px;font-weight:700;color:var(--txt-1);display:flex;align-items:center;gap:8px;"><span class="msi" style="font-size:20px;color:var(--txt-brand);">ballot</span>กระดาษคำตอบ</div>'
    +'<div id="pdfAnsCount" style="font-size:12px;color:var(--txt-3);margin-top:4px;">ตอบแล้ว '+answered+'/'+qs.length+' ข้อ</div>'
    +'</div>'
    +'<div style="flex:1;overflow-y:auto;padding:8px 14px;" data-scroll-id="pdfsheet">'+rows+'</div>'
    +'<div style="padding:14px 18px;border-top:1px solid var(--bdr);">'
    +'<button data-act="submitExam" style="width:100%;padding:13px;background:linear-gradient(135deg,#7B1C1C,#B91C1C);color:#fff;border:none;border-radius:10px;font-size:15px;font-weight:700;cursor:pointer;box-shadow:0 4px 14px rgba(185,28,28,0.3);">ส่งข้อสอบ</button>'
    +'</div>'
    +'</div>';
  return '<div style="display:flex;flex-direction:column;height:100vh;overflow:hidden;">'
    +topbar
